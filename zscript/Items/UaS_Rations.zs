...
class PortableCleanserPack:hdinjectormaker{
	default{
...
//		inventory.pickupmessage "Picked up a Cleanser.";
		...
		scale 0.3;
		hdpickup.bulk ENC_STIMPACK; //!Change
		tag "$TAG_ZERKPACK"; //!Change
		//hdpickup.refid HDLD_BERSERK; //!Change
		+inventory.ishealth
		hdinjectormaker.injectortype "HDCleanser";
	}
	override string PickupMessage() {String pickupmessage = Stringtable.Localize("$PICKUP_ZERKPACK"); return pickupmessage;}
	states{
	spawn:
...
class HDCleanser:HDStimpacker{
	default{
		...
		hdstimpacker.inventorytype "PortableCleanserPack";
		weapon.selectionorder 1003;
		hdstimpacker.injectoricon "CLENA0";
		hdstimpacker.injectortype "PortableCleanserPack";
		tag "$TAG_ZERKPACK";
...
class CleanserDrug:HDDrug{
	...
	override void OnHeartbeat(hdplayerpawn hdp){
		...
		frandom(0,4),frandom(-2,2),frandom(-0.1,0.1),0,
